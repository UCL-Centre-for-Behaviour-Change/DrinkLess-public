...
#import "FollowUpSurveyTableViewController.h"
#import "PXSolidButton.h"
#import "PXFollowUpSurveyManager.h"
#import "PFUser.h"
#import "drinkless-Swift.h"

@interface FollowUpSurveyTableViewController ()
...
@property (nonatomic, weak) PXFollowUpSurveyManager *followUpManager;
@property (nonatomic) NSInteger latestAuditScore;

//@property (nonatomic, assign) BOOL isFemale;  gender-based questions are obsolete
@end

@implementation FollowUpSurveyTableViewController {
...
- (void)viewDidLoad {
    ...
    self.latestAuditScore = AuditData.latest.auditScore;
    ...
    self.followUpManager = [PXFollowUpSurveyManager sharedManager];
...
- (IBAction)pressedContinue:(id)sender {
    ...
    if ([[PXFollowUpSurveyManager sharedManager].answers count] < answerCount) {
    ...
    if (self.screenIndex < 3) {
        ...
        FollowUpSurveyTableViewController *fuVC = [sb instantiateViewControllerWithIdentifier:@"FollowUpID"];
        ...
        resultLbl.text = [NSString stringWithFormat:resultLbl.text, self.latestAuditScore, [self calcAuditScore]];
...
- (void)saveAnswers {
    ...
    if ([PFUser currentUser]) {
        ...
        [pfFollowUpAnswers setObject:@(self.latestAuditScore) forKey:@"OrigScore"];
...
//    NSString *genderKey = [NSString stringWithFormat:@"question-%@", self.isFemale ? @"female" : @"male"];
    NSString *genderKey = [NSString stringWithFormat:@"question-%@", YES ? @"female" : @"male"];
