...
+ (NSString *)uid {
    return [[[PXDeviceUID alloc] initWithKey:@"deviceUID"] uid];
}
 ...
 */
- (void)save {
    if (![PXDeviceUID valueForUserDefaultsKey:_uidKey]) {
        [PXDeviceUID setValue:_uid forUserDefaultsKey:_uidKey];
    }
    if (![PXDeviceUID valueForKeychainKey:_uidKey service:_uidKey]) {
        [PXDeviceUID setValue:_uid forKeychainKey:_uidKey inService:_uidKey];
    }
}
