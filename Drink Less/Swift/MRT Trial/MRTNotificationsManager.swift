...
//    private let TRIAL_NOTIFICATIONS_CUTOFF_DATECOMPS = DateComponents(year: 2019, month: 12, day: 15)
    private let TRIAL_REGISTRATION_CUTOFF_DATECOMPS = DateComponents(year: 2020, month: 3, day: 1)
    private let TRIAL_NOTIFICATIONS_CUTOFF_DATECOMPS = DateComponents(year: 2020, month: 4, day: 1)
     ...
     */
    @objc public func handleMRTNotification(notification:UILocalNotification) -> Bool {
        // Detect MRT notif. Ensure we have the right details
        guard let userInfo = notification.userInfo,
            let notifType = userInfo[KEY_LOCALNOTIFICATION_TYPE] as? String,
    ...
    public func launch(isFirstRun:Bool) {
        ...
        guard hasBeenThroughEligibilityCheck else {
            ...
            var ref:NSObjectProtocol?
            ref = NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: "PXFinishIntro"), object: nil, queue: nil) { (n:Notification) in
        ...
        UNUserNotificationCenter.current().getNotificationSettings() {
            ...
            if isEligible {
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 15.seconds, execute: {
                    if Debug.ENABLED {
                        AlertManager.shared.showSimpleAlert(title: nil, msg: "DEBUG: User randomised to \(trial ?? "(nil)") trial")
                    }
                })
            }
